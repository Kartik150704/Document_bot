...
class ImageScanner:
    ...
    def __init__(self, image, destination, show_results,index):
        """
    ...
    def __analyze_contours(self):
        ...
        # image_opencv = cv2.imread(self.image)
        ...
        (h,w,r) = cv2_image.shape
        if screenCnt is None:
            self.user_defined_contours.append([0, 0])
            self.user_defined_contours.append([0,h])
            self.user_defined_contours.append([w, 0])
            self.user_defined_contours.append([w, h])
            screenCnt = np.array(self.user_defined_contours)   
    ...
    def __save_as_pdf(self, img_data):
        ...
        # cv2.imwrite(f"{self.destination}/",img_data)
        # print(img_data)
        """
        ...
        path_save=f"{self.destination}/Omage{self.index}.jpg"
        im.save(path_save)
